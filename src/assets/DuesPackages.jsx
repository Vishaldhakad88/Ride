...
const DuePackages = () => {
  ...
  const fetchCategories = async () => {
    ...
    if (!token) {
      setError("No token found. Please log in as an organization.");
    ...
    try {
      ...
      if (categoryData.length === 0) {
        toast.info("No categories found for this organization.", {
          position: "top-right",
          autoClose: 3000,
          theme: "colored",
        });
  ...
  useEffect(() => {
    const fetchTenants = async () => {
      ...
      try {
        ...
        if (tenantData.length === 0) {
          toast.warn("No tenants found for this organization.", {
            position: "top-right",
            autoClose: 3000,
            theme: "colored",
          });
  ...
  useEffect(() => {
    const fetchLandlordId = async () => {
      ...
      try {
        ...
        const storedLandlordId = response.data.id;
        console.log("Fetched Organization ID:", storedLandlordId);
        if (storedLandlordId) {
        ...
        toast.error("Failed to fetch organization ID. Please try again.", {
          position: "top-right",
          autoClose: 3000,
          theme: "colored",
        });
  ...
  const handleDuesSubmit = async (e) => {
    ...
    if (!landlordId || typeof landlordId !== "string" || landlordId.trim() === "") {
      ...
      toast.error("Organization ID is invalid or missing. Please log in again.", {
        position: "top-right",
        autoClose: 3000,
        theme: "colored",
      });
